...
prompt_igloo_setup() {
  ...
  local dir
  if [[ $(typeset -f __git_ps1) && $(command git rev-parse --is-inside-work-tree 2>/dev/null) == true && -d .git ]]; then
    #git repo root level
    #short directories
    dir="${dir}%1~"
  elif [[ $(typeset -f __git_ps1) && $(command git rev-parse --is-inside-work-tree 2>/dev/null) == true && -d ../.git ]]; then
    #one level inside git repo
    #medium directories (no dots, only one level)
    dir="${dir}%4(c:%2c"
  elif [[ $(typeset -f __git_ps1) && $(command git rev-parse --is-inside-work-tree 2>/dev/null) == true && -d ../../.git ]]; then
    #two levels inside git repo
    #medium directories (no dots)
    dir="${dir}%4(c:%3c"
  else
  ...
  local seg_working_dir="${nord3}[${nord9}${dir}${nord3}]${newline}${c_reset}"

  local seg_time="${nord3}[${nord9}%D{%H:%M}${nord3}]${newline}${CONNECTBAR_UP}${c_reset}"
