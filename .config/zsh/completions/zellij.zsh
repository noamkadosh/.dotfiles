...
'*--default-mode=[Set the default mode]:DEFAULT_MODE:((normal\:"In `Normal` mode, input is always written to the terminal, except fo
locked\:"In `Locked` mode, input is always written to the terminal and all shortcuts are disabled except the one leading back to nor
...
'*--mouse-mode=[Set the handling of mouse events (true or false) Can be temporarily bypassed by the \[SHIFT\] key]:MOUSE_MODE:(true
